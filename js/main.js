$( document ).ready(function() {
	...
	for(var i=0, j=1; i<len; i++, j++){
		$('table').append('<tr id="wedge_'+i+'"><td id="no_'+i+'">Wedge #'+j+'</td><td>Label:</td><td><input type="text" id="label_'+i+'" value="'+wedges[i].label+'" placeholder="'+wedges[i].label+'"></td><td>Value:</td><td><input type="text" id="value_'+i+'" value="'+wedges[i].value+'" placeholder="'+wedges[i].value+'"></td><td>Color:</td><td><input type="text" id="color_'+i+'" value="'+wedges[i].color+'" placeholder="'+wedges[i].color+'"></td><td><button id="up_'+i+'" onclick="updateWedge('+i+')">Update</button></td><td><button id="rem_'+i+'" onclick="removeWedge('+i+')">Remove</button></td></tr>');
	}
	addWedge();
});

var wedges=[
	{label:"Basic", value:25, color:"#DA9E90"},
	{label:"Plus", value:25, color:"#86C44F"},
	{label:"Deluxe", value:35, color:"#909ADA"},
	{label:"Elite", value:35, color:"#A066CC"},
];

var len = wedges.length;

function addWedge(){
	$('table').append('<tr id="wedge_'+len+'"><td id="no_'+len+'">Wedge #'+(len+1)+'</td><td>Label:</td><td><input type="text" id="label_'+len+'" value="" placeholder=""></td><td>Value:</td><td><input type="text" id="value_'+len+'" value="" placeholder=""></td><td>Color:</td><td><input type="text" id="color_'+len+'" value="" placeholder=""></td><td id="sub" colspan="2"><button id="sub_button" onclick="submitWedge('+len+')">Add Wedge</button></td></tr>');
		wedges.push({
		label: '',
		value: '',
		color: ''
